...
async def send_image_lssv(session: CommandSession):
    ...
    previous_services_data = get_json_data(
        file_path=data_path + "/previous_services_data.json"
    )
    previous_services_data = (
        previous_services_data[group_id]
        if (
            previous_services_data is not None
            and group_id in previous_services_data.keys()
        )
        else {}
    )

    if RANDOM_BG or not os.path.exists(get_path("data", "imgs", "bg.png")):
        await get_random_pic("bg.png")

    try:
        bg_image = Image.open(images_path + "/bg.png")
    except UnidentifiedImageError:
        bg_image = await get_random_pic(file_name="bg.png")
    ...
    write_json_data(
        file_path=data_path + "/previous_services_data.json",
        content={group_id: svs_list},
    )

    sv_image = await get_image_services(
        image=bg_image,
        group_id=group_id,
        sv_list=svs_list,
        width=base_w,
...
@on_command(
    "enable", aliases=("启用", "打开", "开启"), permission=perm.GROUP, only_to_me=False
)
